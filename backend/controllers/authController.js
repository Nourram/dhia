...
const registerAdmin = async (req, res) => {
  try {
    ...
    res.status(201).json({ token, userType: admin.userType });
  } catch (error) {
    console.error(error);
...
const loginAdmin = async (req, res) => {
  try {
    ...
    res.json({ token, userType: admin.userType });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
...
const loginUser = async (req, res) => {
  try {
    ...
    let user = await Parent.findOne({ email }) || await Pedagogue.findOne({ email }) || await HealthCare.findOne({ email });
